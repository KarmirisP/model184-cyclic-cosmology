...
The source-exact merger is solved numerically in a 695-dimensional representation.  At the canonical $\mu_A=7$ handoff we find $t_{\rm match}=-3.97664237$, $k_{\rm match}=1.87484842\times10^{-2}\mpl$, a minimum tensor denominator of $0.645538$, and a maximum componentwise ordinary-differential-equation defect of $7.68\times10^{-7}$.  Dense off-grid projection tests give a maximum polynomial-projector residual of $9.84\times10^{-7}$.  We distinguish Wilson-coefficient displacements from post-matching errors: the largest retained coefficient displacement is $5.89\times10^{-4}$, whereas the reported unrepresented derivative remainder is $5.21\times10^{-5}$.

Across the nonsingular bounce, scalar and tensor perturbations admit regular canonical variables and finite transfer matrices.  Independent recomputation reproduces a scalar transfer amplitude $0.93968314$, corresponding to $P_{\cal R}^{+}/P_{\cal R}^{-}=0.8830044$ on observable scales, with symplectic residual $5.2\times10^{-13}$ and Radau--DOP853 transfer disagreement $2.8\times10^{-11}$; the tensor transfer is consistent with the identity at the tested precision.  A modified CLASS implementation is then sampled with four MCMC chains.  An independent raw-chain replay gives $\max(R-1)=0.005105$ after burn-in.  Held-out ACT DR6 CMB lensing, twelve $f\sigma_8$ measurements, and eight $E_G$ measurements give $\Delta\chi^2=+0.170$ relative to a matched $\LCDM$ comparator over the 30-point validation vector, that is, no statistically meaningful preference in either direction.
...
A reciprocal replay found a discrepancy in an archived posterior-predictive aggregate: one pipeline reports $S_8=0.80896\pm0.00787$, whereas the direct independent CLASS propagation of 250 deterministic raw-chain samples gives $S_8=0.81883\pm0.00829$.  The latter is consistent with the independently reconstructed best-fit value $S_8=0.81713$, while the difference of the means is much larger than the quoted Monte-Carlo standard error of the 250-sample replay.  We therefore exclude the archived $0.80896$ aggregate from all headline conclusions.  The likelihood-level ACT, $f\sigma_8$, and $E_G$ replay results are unaffected and agree between machines at the $10^{-3}$--$10^{-4}$ level in $\Delta\chi^2$.
...
The final public release should contain the exact manuscript source, BibTeX database, frozen theory and observational authority archives, SHA-256 manifests, modified CLASS source, raw MCMC chains, likelihood configuration files, and deterministic replay scripts.  During peer review, repository/DOI identifiers should be inserted here only after the corresponding immutable public release has been created.  No unpublished private data are required to reproduce the calculations reported in this paper.
...
\end{tabular}
\end{center}
The $W$ entry is explicitly labelled as a retained coefficient displacement.  The independent omitted-derivative estimate is $R_{\rm trunc}=5.21\times10^{-5}$.  This appendix fixes the convention needed to prevent a Wilson coefficient from being misidentified as a post-match residual.
